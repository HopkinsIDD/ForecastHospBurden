...
ggplot(covid_US, aes(x = date, y = incidH_prior_day)) +
  geom_line() +
  labs(
...
```{r}
# run this multiple times to visualize how the daily estimates for length of stay change introducing variability into hospital burden estimates on the same set of data 
plot_one_run <- function(incidH = 150,
                         origin = as_date("2025-01-01"),
                         los = 5) {
  # Expand one-day admissions into occupied-bed dates
  dates <- burden_est_funct(incidH = incidH, date = origin, los = los)

  # Count current hospitalizations by calendar date
  occupancy <- tibble(date = dates) |>
    filter(date >= origin) |>
    count(date, name = "current_hosp")

  ggplot(occupancy, aes(date, current_hosp)) +
    geom_line() +
    geom_point(size = 0.8) +
    labs(
      title = "Single run: occupancy from one-day admissions",
      subtitle = paste0("incidH = ", incidH,
                        ", LOS ~ NegBin(size = ", los, ", prob = 0.5)"),
      x = NULL, y = "Current hospitalizations"
    ) +
    theme_minimal(base_size = 12)
}

# Run once; re-run this line to see a new random path each time:
plot_one_run(incidH = 150, origin = as_date("2025-01-01"), los = 5)
```
...
create_totalH_df_by_factor(data = covid_HHS_data_states_lagtemp %>% dplyr::select(-incidH, -incidH_prior_day), factor_col = "season")
