...
# update function to just use lit estimate of 3.9 / 5.0
create_totalHosp_data <- function(parent_data){
  states_list <- unique(parent_data$state)
  combined_list <- list()
  ...
  for (state in states_list) {
    ...
    expected_list <- create_hosp_dates(data, los = 5.0)
    expected <- create_curr_hosp(data_burden = expected_list)
...
# Write Final files ----------------
covid_joined_totalHosp_state_data <- create_totalHosp_data(parent_data = covid_HHS_data_states_lag)

sqr()
# Calculate RMSE
rmse <- sqrt(sum((covid_joined_totalHosp_state_data$total_hosp - covid_joined_totalHosp_state_data$total_hosp_estimate)^2))
MAE <- sum(abs(covid_joined_totalHosp_state_data$total_hosp - covid_joined_totalHosp_state_data$total_hosp_estimate))
