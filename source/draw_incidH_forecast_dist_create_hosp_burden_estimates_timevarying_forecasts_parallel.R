...
# all_states_samples <- read_parquet("data/US_wide_data/forecast_hosp_all_dates_horizon_sampled_incidH_from_quantile_01292025.parquet")
...
#   )
# 
# write_parquet(all_states_samples_los, "data/US_wide_data/all_states_samples_los_pois_03192025.csv")

all_states_samples_los <- read_parquet("data/US_wide_data/all_states_samples_los_pois_03192025.csv")
# # join prior reported hospitalizations for each forecast date
# MD_samples <- forecast_hosp_MD_samples_los %>% 
#   filter(location_name == "Maryland",
#          simulation == 1) 
# 
# hosp_MD <- covid_HHS_data_states_lag %>% 
#   filter(state == "MD") 
# 
# 
# forecast_date_list <- unique(MD_samples$forecast_date)
# origin_date <- as.Date("2023-08-28")
# origin_date <- as.Date("2023-10-02")
# 
# read_in_reported_data_up_to_forecast_date <- function(forecast_date, hosp_MD) {
#   for(origin_date in forecast_date_list){
#     
#     
#   hosp_MD_prior_month <- hosp_MD %>% 
#     filter(date <= forecast_date & date >= forecast_date - 30) %>% 
#     mutate(abbreviation = state,
#            state = unique(MD_samples$state)) 
#   return(hosp_MD_prior_month)
#   }
# }
# 
# for(origin_date in forecast_date_list){
...
#     mutate(abbreviation = state,
#            state = unique(MD_samples_origin$state)) 
# 
#   forecast_date_join <- bind_rows(MD_samples_origin, hosp_MD_prior_month)
#   forecast_date_leftjoin <- MD_samples_origin %>%
#     left_join(hosp_MD_prior_month %>% select(everything(), incidH_reported = incidH), by = c("date", "abbreviation", "state")) %>%
#     mutate(incidH = if_else(is.na(incidH), incidH_reported, incidH))
#   
#   MD_samples <- MD_samples %>% 
#     left_join(total_hosp_MD %>% filter(forecast_date == origin_date) %>% select(date, total_hosp), by = c("target_end_date" = "date")) %>% 
#     rename(total_hosp_prior_day = total_hosp)
#   
#   
# }

# forecast_hosp_MD_samples_los_trial <- all_states_samples_los %>% 
...
#, simulation %in% c(1, 2)
start_time <- Sys.time()
optimized_data_all_quantiles <- create_optimize_totalHosp_data_timevarying_forecast_simulations_parallel2(
  parent_data = all_states_samples_los_MD, covid_HHS_data_states_lag)
end_time <- Sys.time()
...
write_parquet(covid_joined_totalHosp_state_data_los_demographic, "data/US_wide_data/estimated_hospitalizations_data/Obs_Exp_totalHosp_daily_TIMEVARYING_FORECAST_SIMU_quantiles_14days_parallel03192025_MD.parquet")
